...
const walletPath = 'C:/Users/ahmed/.fabric-vscode/environments/Local Fabric/wallets/Org1';
exports.connectToNetwork = async function (userName) {

  const gateway = new Gateway();

  try {
    //const walletPath = path.join(process.cwd(), 'peer-wallet');
...
exports.registerParticipant = async function (id, name, email, national_id, address, city, zip_code, password, created_at) {
  ...
  try {
    ...
    if (userExists) {
      ...
      response.error = `Error! An identity for the user ${id} already exists in the wallet. Please enter
    ...
    const secret = await ca.register({ affiliation: 'org1', enrollmentID: id, role: 'client' }, adminIdentity);
